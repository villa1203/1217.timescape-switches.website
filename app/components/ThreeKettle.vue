...
const NORMAL_METAL = 0xf5f5f5     // normal — light aluminum finish
const NORMAL_BLACK = 0x000000  // normal — black plastic parts

/* ─────────────────────────── part assignments ─────────────────────── */
// Parts are assigned by mesh name on load (see console.log below).
// purpleParts:  glow PURPLE in xray mode
// blackParts:   black plastic in normal mode
//   note: some parts appear in both (black in normal, purple in xray)
let purpleParts = []
let blackParts  = []
  ...
  loader.load(
    ...
    (gltf) => {
      ...
      normalMeshes.forEach((child, i) => {
        ...
        // Purple glow in xray — these are the internal heating elements / indicator rings
        ...
        // Black plastic parts — also includes circle002_1 (black in normal, purple in xray)
        if (name === 'circle' ||
            name.includes('circle002_1') ||
      ...
      // console.log('ThreeKettle meshes:')
      // normalMeshes.forEach((c, i) => console.log(`  [${i}] name="${c.name}" parent="${c.parent?.name}"`))
      // console.log('ThreeKettle part assignments:', { purpleParts, blackParts })
      ...
      normalMeshes.forEach((child, i) => {
        ...
        // blackParts covers both pure-black parts and those that glow purple in xray
        const mat = blackParts.includes(i)
